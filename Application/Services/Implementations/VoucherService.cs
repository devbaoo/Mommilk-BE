        ...
        : BaseService(unitOfWork, mapper), IVoucherService
    {
        ...
        public async Task<IActionResult> GetValidVouchers()
        {
            var vouchers = await _voucherRepository
                .Where(v => v.Status.Equals(VoucherStatuses.ACTIVE) && v.From < DateTimeHelper.VnNow && v.To > DateTimeHelper.VnNow && v.Quantity > 0)
                .ProjectTo<VoucherViewModel>(_mapper.ConfigurationProvider)
                .ToListAsync();
            return vouchers.Ok();
        }
        ...
        {
            if (model.From >= model.To || model.From.Date < DateTimeHelper.VnNow.Date)
            {
